...
"$dotnetpath" restore "../Metalinker.sln" -p:Configuration=$releaseconf ${@:2}
checkerror $? "Failed to download packages"

# Build
echo Building...
"$dotnetpath" build "../Metalinker.sln" -p:Configuration=$releaseconf ${@:2}
checkerror $? "Failed to build Metalinker"
